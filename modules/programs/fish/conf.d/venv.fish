...
function __auto_source_venv --on-variable PWD --description "Activate/Deactivate virtualenv on directory change"
    ...
    # if in devpod environment (defined by environment var DEVPOD = true), use the devpod venv in .pod-venv
    if test -n "$DEVPOD" -a "$DEVPOD" = "true"
        if test -e "$dir/.pod-venv/bin/activate.fish"
            set venv_dir "$dir/.pod-venv"
        end
    else
